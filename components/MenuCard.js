            ...
            {qty > 0 ? (
              ...
              >
                <button
                  ...
                  className="flex h-6 w-6 items-center justify-center rounded-full text-white hover:bg-white/15 active:scale-75 transition-spring text-xs font-bold"
                >
                  -
                </button>
                <span className="w-4 text-center text-[11px] font-bold animate-badge-pop">{qty}</span>
                <button
              ...
              <button
                onClick={handleAddClick}
                className="flex h-7 items-center gap-1 rounded-full bg-white px-3 text-[11px] font-bold text-[var(--ink)] shadow-md border border-[var(--line)] hover:bg-[var(--accent)] hover:text-white hover:border-[var(--accent)] active:scale-90 transition-spring"
              >
                <span>+</span>
                <span>Add</span>
              </button>
      ...
      <article className="group hidden md:flex flex-col overflow-hidden rounded-2xl border border-[var(--line)] bg-[var(--card)] card-hover-effect">
        ...
        <div className="relative aspect-[4/3] w-full overflow-hidden bg-[var(--accent-soft)]">
          {item.image ? (
            ...
            <div className="flex h-full w-full items-center justify-center">
              <CrownMark className="h-10 w-10 text-[var(--accent)] opacity-40 transition-transform duration-500 group-hover:scale-110" />
            </div>
          )}
            ...
            <div className="absolute bottom-2.5 right-2.5 z-10">
              {qty > 0 ? (
                ...
                >
                  <button
                    onClick={handleMinusClick}
                    className="flex h-7 w-7 items-center justify-center rounded-full text-white hover:bg-white/15 active:scale-75 transition-spring text-sm font-bold"
                  >
                    -
                  </button>
                  <span className="w-5 text-center text-xs font-bold animate-badge-pop">{qty}</span>
                  <button
                    onClick={handleAddClick}
                    className="flex h-7 w-7 items-center justify-center rounded-full text-white hover:bg-white/15 active:scale-75 transition-spring text-sm font-bold"
                  >
                ...
                <button
                  onClick={handleAddClick}
                  className="flex h-8.5 items-center gap-1.5 rounded-full bg-white/95 backdrop-blur-xs px-3.5 text-xs font-bold text-[var(--ink)] shadow-lg border border-[var(--line)] transition-spring hover:bg-[var(--accent)] hover:text-white hover:border-[var(--accent)] active:scale-90"
                >
                  <span>+</span>
                  <span>Add</span>
                </button>
        ...
        <div className="flex flex-1 flex-col gap-1 sm:gap-2 p-3 sm:p-5">
          <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-1 sm:gap-3">
            <h3 className="font-display text-sm sm:text-lg leading-snug group-hover:text-[var(--accent)] transition-colors duration-300">{item.name}</h3>
