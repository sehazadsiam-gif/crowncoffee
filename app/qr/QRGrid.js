...
export default function QRGrid({ baseUrl, tableCount = 50 }) {
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={`https://api.qrserver.com/v1/create-qr-code/?size=260x260&data=${encodeURIComponent(`${origin}/order?type=tab`)}&margin=12&color=1C1612&bgcolor=FFFBF5`}
            alt="QR code for Tab Order"
            className="h-40 w-40 rounded-lg"
            loading="lazy"
          />
          ...
          {/* Label */}
          <div className="mt-3 flex flex-col items-center gap-0.5">
            <span className="text-[10px] font-semibold tracking-[0.2em] uppercase text-[var(--ink-soft)] font-mono">Tab</span>
            <span className="font-display text-lg font-black text-[var(--accent)] uppercase text-center leading-tight">Tab Order</span>
          </div>

          {/* URL hint */}
          <p className="mt-2 text-[9px] text-center text-[var(--ink-soft)] leading-tight break-all">
            {origin}/order?type=tab
          </p>
