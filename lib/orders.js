...
/**
 * Push an order-deleted event to all connected SSE clients.
 * Carries the orderId so customers can detect removal immediately.
 */
export function broadcastOrderDeleted(orderId) {
  const payload = `data: ${JSON.stringify({ type: "order_deleted", orderId })}\n\n`;
  for (const writer of globalThis._orderClients) {
    try {
      writer.write(payload);
    } catch {
