...
import INITIAL_MENU from "@/data/menu.json";
import INITIAL_SETTINGS from "@/data/settings.json";
import INITIAL_BANNERS from "@/data/banners.json";
import INITIAL_NOTICES from "@/data/notices.json";
...
const NOTICES_BLOB_KEY = "data/notices.json";
...
export async function readJsonBlob(key, fallback) {
  try {
    ...
    if (nativeR2) {
      const obj = await nativeR2.get(key);
      ...
      try {
        ...
        console.error(`Malformed JSON in R2 for key ${key}:`, parseErr);
        return fallback;
      }
    }
    ...
    console.error(`Error reading R2 blob for key ${key}:`, err);
    return fallback;
  }
}
...
async function writeJsonBlob(key, data) {
  try {
    const nativeR2 = getNativeR2();
    if (nativeR2) {
      await nativeR2.put(key, JSON.stringify(data, null, 2), {
...
async function listBlobs(prefix) {
  try {
    const nativeR2 = getNativeR2();
    if (nativeR2) {
      const list = await nativeR2.list({ prefix });
      return (list.objects || []).map((obj) => ({
        pathname: obj.key,
        url: `${process.env.R2_PUBLIC_URL?.trim()}/${obj.key}`,
      }));
    }
    ...
    console.error(`Error listing blobs for prefix ${prefix}:`, err);
    return [];
  }
}
...
async function writeJson(key, filePath, data) {
  if (useBlob()) return writeJsonBlob(key, data);
...
export async function getMenu() {
  noStore();
  return readJson(MENU_BLOB_KEY, MENU_FILE, INITIAL_MENU);
}
...
export async function getSettings() {
  noStore();
  const settings = await readJson(SETTINGS_BLOB_KEY, SETTINGS_FILE, INITIAL_SETTINGS);
  return {
...
export async function getBanners() {
  noStore();
  const data = await readJson(BANNERS_BLOB_KEY, BANNERS_FILE, INITIAL_BANNERS);
...
export async function getNotices() {
  noStore();
  const data = await readJson(NOTICES_BLOB_KEY, NOTICES_FILE, INITIAL_NOTICES);
  return { notices: Array.isArray(data.notices) ? data.notices : [] };
}

export async function saveNotices(data) {
  return writeJson(NOTICES_BLOB_KEY, NOTICES_FILE, data);
}

// ─── Orders ───────────────────────────────────────────────────────────────────
let lastReconciliationTime = 0;
const RECONCILIATION_INTERVAL_MS = 5 * 60 * 1000; // Reconcile missing files at most once every 5 minutes
...
export async function getOrders() {
  ...
  // 2. Only run individual order file reconciliation if cache is expired or store is empty
  const now = Date.now();
  if (orders.length === 0 || now - lastReconciliationTime > RECONCILIATION_INTERVAL_MS) {
    ...
    try {
      ...
      if (useBlob()) {
        ...
        const missingBlobs = blobs.filter((b) => {
          ...
          return match ? !orderIdSet.has(match[1]) : false;
        });

        if (missingBlobs.length > 0) {
          const fetched = await Promise.all(
            missingBlobs.map((b) => readJsonBlob(b.pathname, null))
          );
